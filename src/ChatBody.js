import React, { useEffect } from 'react'
import { useState } from 'react'
import Message from './Messages'
import { chatBot, chatBotArr } from './ChatBot'
export default function ChatBody() {
    const [textInput, setTextInput] = useState("")
    const [userChat, setUserChat] = useState(["asdasd", "asdasdasd", "ayxcyxyx"])
    const [chat, setChat] = useState([
             
    ])
    const classesForMain = ["justify-content-end", "msg_cotainer_send", 'deleteContainer1', "action_menu_delete", "msg_time_send", 'arrow-down1']
    const classesForOther = ["justify-content-start", "msg_cotainer", 'deleteContainer', "action_menu_delete_otherUser", "msg_time", 'arrow-down']
    // const chatBot = [[{value: "how are you ?"}, "im good thanks, what about you dear ? "], [{value: "how is your day ?"}, "good "]]
 


   const saveMsgInput = (event)=>{
     const value = event.target.value
     setTextInput(value)

   }
   const saveMsgToChat = ()=>{
    setUserChat(prev=>[...prev, textInput])
    console.log(userChat);
  }

  const submitMsg = ()=>{
    if(textInput !== ""){
        var matchedWords = 0
        var InfoOb = {}
        var infoArr = []
        var today = new Date()
        var time = today.getHours() + ":" + today.getMinutes() 
        setChat(prev=>[...prev, {
            arthur: "main",
            content: textInput,
            time: time
        }])
        
        const test = ()=>{
            var check = false
            chatBotArr.map((ele, wordIndex)=>{
                textInput.split(" ").map(word1=>{
                    ele.map(word2=>{
                        if(word1 === word2){
                            matchedWords ++
                        }
                    })
                })
                InfoOb ={
                    wordIndex: wordIndex,
                    matchPercentage: matchedWords / ele.length
                }
                infoArr.push(InfoOb)
                matchedWords = 0
            })
            var mostMatchedIndex = 0
            var mostMatchedPercentage = 0
            
            infoArr.map(ele=>{

                if(mostMatchedPercentage < ele.matchPercentage ){
                     mostMatchedPercentage = ele.matchPercentage
                     mostMatchedIndex = ele.wordIndex
                }
            })

           
            if (mostMatchedPercentage < 0.5) {
                setChat(prev=>[...prev, {
                    arthur: "bot",
                    content: "I'm sorry i didn't understand that!",
                    time: time
                }]) 
            }else{
                setChat(prev=>[...prev, {
                    arthur: "bot",
                    content: chatBot[mostMatchedIndex].value,
                    time: time
                }])
            }

            
            
            // chatBot.map((ele, index)=>{
            //     console.log(ele[0].pos);
            //     setChat(prev=>[...prev, {
            //         arthur: "bot",
            //         content: ele[0].possible,
            //         time: time
            //     }])
            //     check = true
            // })


            // chatBot.map((ele)=>{
            //     if(ele.possible === textInput ){
                    
            //         setChat(prev=>[...prev, {
            //             arthur: "bot",
            //             content: ele.value,
            //             time: time
            //         }])
            //         check = true
            //     } 
            // })
            // if(check === false){
            //     setChat(prev=>[...prev, {
            //         arthur: "bot",
            //         content: "I'm Sorry i didn't understand that!",
            //         time: time
            //     }])
            // } 
        }
        setTimeout(test, 800)
        setTextInput("")
    }
  }


  return (
    <div className="container-fluid h-100">
    <div className="row justify-content-center h-100">
        
        <div className="col-md-8 col-xl-6 chat" style={{marginTop: "40px"}}>
            <div className="card">
                <div className="card-header msg_head">
                    <div className="d-flex bd-highlight">
                        <div className="img_cont">
                            <img src="data:image/png;base64," className="rounded-circle user_img"/>
                            <span className="online_icon"></span>
                        </div>
                        <div className="user_info">
                            <span>Chat Bot</span>
                            {/* <p>1767 Messages</p> */}
                        </div>
                        {/* <div className="video_cam">
                            <span><i className="fas fa-video"></i></span>
                            <span><i className="fas fa-phone"></i></span>
                        </div> */}
                    </div>
                    <span id="action_menu_btn"><i className="fas fa-ellipsis-v"></i></span>
                    <div className="action_menu">
                        <ul>
                            <li><i className="fas fa-user-circle"></i> View profile</li>
                            <li><i className="fas fa-users"></i> Add to close friends</li>
                            <li><i className="fas fa-plus"></i> Add to group</li>
                            <li><i className="fas fa-ban"></i> Block</li>
                        </ul>
                    </div>
                </div>
                <div className="card-body msg_card_body">
                
                {/* {useEffect(()=>{
                    {chat.map((msg, index)=>{
                        
                        if(msg.arthur === "main"){
                            <Message key={index} whichOne={"main"} classes={classesForMain}
                        time={msg.time} content={msg.content} 
                        name ={"asd"}  />
                        }else{
                            <Message key={index} whichOne={"bot"} classes={classesForOther}
                        time={msg.time} content={msg.content} 
                        name ={"asd"}  />
                        }
                   })}
                }, [chat])} */}

              

                {chat.map((msg, index)=>{
                       
                       if(msg.arthur === "main"){
                            return(
                                <Message key={index} whichOne={"main"} classes={classesForMain}
                                time={msg.time} content={msg.content} 
                                name ={"asd"}  />
                            )    
                        }else{
                            return(
                                <Message key={index} whichOne={"bot"} classes={classesForOther}
                                time={msg.time} content={msg.content} 
                                name ={"asd"}  />
                            )
                        }
                   })}
                
                </div>
                <div className="card-footer">
                    <div className="input-group">
                        
                        <textarea onChange={saveMsgInput} value={textInput} name="msgContent" className="form-control type_msg" placeholder="Type your message..."></textarea>
                        <div className="input-group-append">
                            <span className="input-group-text send_btn"><i onClick={submitMsg} className="fas fa-location-arrow"></i></span>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
  )
}
